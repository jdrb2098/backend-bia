...
def getAuditorias(request):
    ...
    try:
        ...
        print(auditoria_persona_id)
        print(auditoria_usuario_id)
        try:
            modulo = Modulos.objects.get(id_modulo=modulo).id_modulo
            try:
                subsistema = Modulos.objects.get(subsistema=subsistema).subsistema
        ...
        try:
            modulos = Modulos.objects.get(id_modulo=modulo).id_modulo
            try:
                subsistema = Modulos.objects.get(subsistema=subsistema).subsistema
                auditorias = Auditorias.objects.distinct().filter(
                    Q(id_modulo=int(modulos))&
                    Q(subsistema=subsistema)&
                    Q(fecha_accion__range=[start_date,end_date])).order_by('-fecha_accion')
                page = request.query_params.get('page')
                paginator = Paginator(auditorias, 10)
